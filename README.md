...
For each set of identical files, the script makes every displayed media filename a hyperlink that opens it in VS Code. Select an original file number to retain it in place. Further numbered options combine the location from one file with the filename from another: for two files, option `3` is location 1 with filename 2 and option `4` is location 2 with filename 1. With more copies, every cross-location/filename combination is listed. The selected copy is moved when necessary and all Markdown references are updated. Enter `s` to skip the set or `q` at any prompt to quit.
